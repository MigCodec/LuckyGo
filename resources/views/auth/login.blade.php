...
<body>
    ...
    <h1 style="margin-bottom: 16px; font-size: 2.5rem; font-weight: 800; line-height: 1.5; text-align: center;">Iniciar sesión</h1>
        ...
        <form style="max-width: 20rem; margin: 0 auto;" method="POST" action="{{ route('login') }}" novalidate>
          @csrf
            ...
            <div style="margin-top: 1rem; text-align: center;">
              <button type="submit" style="margin-bottom: 10px; background-color: #3B82F6; color: #fff; font-weight: 500; border-radius: 0.375rem; font-size: 0.875rem; text-align: center; padding: 0.625rem 1.25rem; width: 100%; max-width: 10rem;">Acceder</button>
              @if(session('login_successfuly'))
              <p style="background-color: #2ECC71; color: #fff; border-radius: 0.375rem; font-size: 1rem; padding: 0.25rem;">{{session('login_successfuly')}}</p>
              @endif
              @if(session('message'))
              <p style="background-color: #f56558; color: #fff; border-radius: 0.375rem; font-size: 1rem; padding: 0.25rem;">{{session('message')}}</p>
              @endif
            </div>
        </form>        
    </div>
